...
export class FormularioVMService {
  ...
  send() {
    alert(
      (this.modo === 'add' ? 'POST ' : 'PUT ') + JSON.stringify(this.elemento)
    );
    if (this.modo == 'add') {
      this.http
        .post(`http://localhost:4321/api/personas`, this.elemento)
        .subscribe({
          next: (data) => {
            alert('OK');
          },
      ...
      this.http
        .put(
          `http://localhost:4321/api/personas/${this.elemento.id}`,
          this.elemento
        )
        .subscribe({
          next: (data) => {
